...
Route::group(['prefix' => env('BASE_URL', ''), 'middleware' => 'web'], function () {
    if (config('larams.register_image_routes')) {
    ...
    if (config('larams.register_admin_routes')) {
        Route::get(config('larams.admin.prefix'), 'Admin\AuthController@getLogin')->middleware('ipCheck');

        Route::group(['prefix' => config('larams.admin.prefix'), 'namespace' => 'Admin', 'middleware' => ['ipCheck', 'cacheControl', 'permission']], function () {
            ...
            Route::group(['middleware' => config('larams.admin.guard_middleware')], function () {
