...
class TypeController extends Controller
{
    ...
    public function anyIndex( StructureItem $structureItem, StructureType $structureType, Request $request )
    {
        ...
        if ( empty( $uri ) || (!empty( $currLang ) && $uri == $currLang->uri) ) {
            ...
            $currItem  = $structureItem->withTrashed()->where('left', '>', $currLang->left)->where('right', '<', $currLang->right)->with('type')->where('uri', $uri )->orderBy('deleted_at', 'asc')->orderBy('active', 'desc')->first();
